...
protected:
    ...
    std::pair<bool, InstructionStream> compileAndGetInstructions(const std::string& source) {
        auto result = compiler.compile(source);
...
TEST_F(CompilerTest, DeclareIntVariable) {
    EXPECT_TRUE(compile("int x = 5;"));
}
...
TEST_F(CompilerTest, DeclareIntArray) {
    EXPECT_TRUE(compile("int arr[10];"));
}
...
TEST_F(CompilerTest, ContinueStatement) {
    auto [ok, instrs] = compileAndGetInstructions(
        "int i = 0; while (i < 10) { i++; continue; }");
    EXPECT_TRUE(ok);
    EXPECT_GE(countOpcode(instrs, Opcode::Jump), 2);
}

TEST_F(CompilerTest, BreakOutsideLoopError) {
    EXPECT_FALSE(compile("break;"));
}
...
TEST_F(CompilerTest, ZeroLiteral) {
    EXPECT_TRUE(compile("int x = 0;"));
}

TEST_F(CompilerTest, NegativeLiteral) {
    EXPECT_TRUE(compile("int x = -42;"));
}

TEST_F(CompilerTest, CharLiteral) {
    EXPECT_TRUE(compile("char c = 'Z';"));
}
