...
class RuntimeTest : public ::testing::Test {
 protected:
  ...
  std::unique_ptr<Program> compileToProgram(const std::string& source) {
    ...
    return std::make_unique<Program>(std::move(result.instructions),
  ...
  RunResult runProgram(Program& program, int maxSteps = 10000) {
    ...
    // Capture output via stepResult.output only (not callback to avoid
    // duplicates)
    ...
    result.memory =
        std::vector<int32_t>(program.memory().begin(), program.memory().end());
    return result;
  }
...
TEST_F(RuntimeTest, IfElseTrueBranch) {
  auto result =
      compileAndRun("int x = 0; if (true) { x = 1; } else { x = 2; }");
  EXPECT_TRUE(result.halted);
  EXPECT_EQ(getMemoryAt(result, 0), 1);
}
...
TEST_F(RuntimeTest, IfElseFalseBranch) {
  auto result =
      compileAndRun("int x = 0; if (false) { x = 1; } else { x = 2; }");
  EXPECT_TRUE(result.halted);
  EXPECT_EQ(getMemoryAt(result, 0), 2);
}
...
class SchedulerTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override { scheduler.clear(); }
};
