...
class IntegrationTest : public ::testing::Test {
 protected:
  ...
  std::string compileAndRun(const std::string& source, int maxSteps = 50000) {
    ...
    Program program(std::move(result.instructions), std::move(result.symbols));
...
class ConcurrencyTest : public ::testing::Test {
 protected:
   ...
   */
  ConcurrencyResult compileAndRunConcurrent(const std::string& source,
                                            int maxSteps = 100000) {
    ...
    auto program =
        std::make_shared<Program>(std::move(compileResult.instructions),
...
TEST_F(ConcurrencyTest, CobeginCoend_LocalVariablesInEachBlock) {
  ...
  // This was a bug: variables declared in one block conflicted with sibling
  // blocks
  std::string source = R"(
        int result1 = 0;
        int result2 = 0;
